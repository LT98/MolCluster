...
exit gate is met, so the active plan stays short enough to read.

Read this when you need to know *what was already decided and why* — the ground rules in
...
Each kept in the form it had when it was written, including its exit gate, so a regression
can be checked against the gate that was actually agreed rather than a remembered version
of it.
...
## 2. Interface architecture (decided and built)
...
## 3. Salvage ledger — what happened to `legacy/`
